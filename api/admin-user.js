...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'bootstrapOwner') {
      ...
      await db.collection('auditLogs').add({ action: 'bootstrap_owner', actorId: actor.id, actorName: actor.name, target: au.uid, details: 'owner123', createdAt: now });
    ...
    if (action === 'createUser') {
      ...
      const now = admin.firestore.Timestamp.now();
      try { await db.runTransaction(async tx => { if ((await tx.get(lockRef)).exists) throw Object.assign(new Error('USERNAME_TAKEN'), { statusCode: 409 }); tx.create(lockRef, { userId: au.uid, createdAt: now }); tx.create(db.collection('users').doc(au.uid), { authUid: au.uid, email, username: usernameNorm, name: String(name).trim(), phone, employeeId, area, branch: role === 'agent' ? branch : '', role, status: 'active', deviceId: null, deviceInfo: null, activeSessionId: null, hasAlert: false, createdAt: now, createdBy: actor.id }); }); } catch (e) { await admin.auth().deleteUser(au.uid); throw e; }
      await db.collection('auditLogs').add({ action: `create_${role}`, actorId: actor.id, actorName: actor.name, target: au.uid, details: usernameNorm, createdAt: now });
    ...
    else if (action === 'setBranch' || action === 'updateProfile') { if (branch !== undefined && !BRANCHES.includes(branch)) return res.status(400).json({ ok: false, error: 'INVALID_BRANCH' }); Object.assign(updates, { ...(branch !== undefined ? { branch } : {}), ...(body.name ? { name: String(body.name).trim() } : {}), ...(body.phone !== undefined ? { phone: String(body.phone) } : {}), ...(body.employeeId !== undefined ? { employeeId: String(body.employeeId) } : {}), ...(body.area !== undefined ? { area: String(body.area) } : {}) }); }
    else if (action === 'deleteUser') { if (uid === actor.id) return res.status(400).json({ ok: false, error: 'CANNOT_DELETE_SELF' }); await admin.auth().deleteUser(uid); const batch = db.batch(); batch.delete(ref); if (user.username) batch.delete(db.collection('usernames').doc(user.username)); batch.delete(db.collection('openAttendance').doc(uid)); const related = await db.collection('attendance').where('agentId', '==', uid).get(); related.docs.forEach(d => batch.delete(d.ref)); const leaves = await db.collection('leaves').where('agentId', '==', uid).get(); leaves.docs.forEach(d => batch.delete(d.ref)); await batch.commit(); await db.collection('auditLogs').add({ action: 'delete_user_permanent', actorId: actor.id, actorName: actor.name, target: uid, details: reason, createdAt: admin.firestore.FieldValue.serverTimestamp() }); return res.status(200).json({ ok: true, deleted: true }); }
