...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    else if (action === 'setBranch' || action === 'updateProfile') { if (branch !== undefined && ((user.role === 'agent' && !BRANCHES.includes(branch)) || (user.role === 'admin' && user.adminLevel !== 'primary' && !BRANCHES.includes(branch)) || (user.role === 'admin' && user.adminLevel === 'primary' && branch !== '' && !BRANCHES.includes(branch)))) return res.status(400).json({ ok: false, error: 'INVALID_BRANCH' }); Object.assign(updates, { ...(branch !== undefined ? { branch } : {}), ...(body.name ? { name: String(body.name).trim() } : {}), ...(body.phone !== undefined ? { phone: String(body.phone) } : {}), ...(body.employeeId !== undefined ? { employeeId: String(body.employeeId) } : {}), ...(body.area !== undefined ? { area: String(body.area) } : {}) }); if (body.permissions && user.role === 'admin') { if (!isOwner(actor)) return res.status(403).json({ ok: false, error: 'OWNER_REQUIRED' }); const allowed = ['attendance','editAgents','createAgents','devices','locations','notifications','reports']; updates.permissions = Object.fromEntries(allowed.map(k => [k, body.permissions[k] === true])); } }
